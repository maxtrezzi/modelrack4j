...
| [P36](post-v1.md#p36--housekeeping-a-test-that-raced-its-own-listener-and-the-first-read-of-five-merged-branches) | Housekeeping: a test that raced its own listener, and the first read of five merged branches | **In progress** — six `ReloadTest` tests read a listener's counter after waiting on the registry, which publishes before it announces; the combined read of `#50`–`#54` found nothing to correct |
