...
| Item | What | Status |
|---|---|---|
| [Task 0.1](phase-0-verification.md#task-01--pin-the-langchain4j-version) | Pin the LangChain4j version | **Done** — `1.20.0`, re-pinned twice |
...
| [P24](post-v1.md#p24--watchtrue-cannot-see-file-backed-layers-given-through-sources) | `watch(true)` cannot see file-backed layers given through `sources(...)` | **Done** — at least one file layer, the rest ignored; the condition had been stricter than the ADR it came from; ADR-0050 |
...
| [P34](post-v1.md#p34--langchain4j-1200-and-the-jar-the-aggregate-started-bringing) | LangChain4j `1.20.0`, and the jar the aggregate started bringing | **Done** — the aggregate stopped being free (`mutiny-zero`, kept on purpose) and the bump needs a `jspecify` pin to build |
...
| [P39](post-v1.md#p39--a-relative-configuration-path-loses-its-sibling-includes) | A relative configuration path loses its sibling includes | **Done** — `Path.of("app.conf")` had no parent, so `parseFile` resolves `include "sibling.conf"` to nothing; an optional key would go missing with no error at all, and the watcher and the write path were unaffected because both absolutise first. Fixed in the compact constructor of both file-backed records, which also repaired the second defect found there: two spellings of one path reported one `id()` and compared unequal |
| [P40](post-v1.md#p40--custom-properties-carried-as-text) | Custom properties, carried as text | **Done** — the text goes in `LlmConfig` for the diff and the parsed object in `LlmBundle`; the handler is generic, so `LlmRegistry`, `LlmBundle` and `LlmSnapshot` gained a type parameter while `LlmConfig` did not, which is why no provider changed. Rendering the merged block kept a key a higher layer had cleared, as an explicit null, and now strips them at every depth; ADR-0055 |
| [P41](post-v1.md#p41--reject-a-key-the-schema-does-not-know) | Reject a key the schema does not know | **Done** — the known keys are produced by the parse recording what it asks for, and one error lists every offending key with its layer and line. A missing required value is now collected rather than thrown on, so the recorded set is complete and a misspelled required key names the misspelling instead of the absence it caused; ADR-0056 |
| [P42](post-v1.md#p42--an-empty-configuration-and-the-layer-that-empties-it) | An empty configuration, and the layer that empties it | **Done** — the last configuration can now be removed, which ADR-0014 always promised; `= null` on a block or on the root is refused in words that say why; and `llm = 5`, found while handling the cases around it, no longer escapes as Typesafe Config's own exception; ADR-0057, ADR-0058 |
...
| [P48](post-v1.md#p48--api-key-and-base-url-what-a-provider-permits-and-requires) | `api-key` and `base-url`: what a provider permits and requires | Not started — target 0.3.0; ADR-0062 |
| [P49](post-v1.md#p49--an-ollama-provider) | An Ollama provider | Blocked — on P48; target 0.3.0 |
...
What the phase settled: the pin (`1.19.0` then, `1.20.0` now) and the two-BOM import it later needed, `release`
...
[P41](post-v1.md#p41--reject-a-key-the-schema-does-not-know).
[P39](post-v1.md#p39--a-relative-configuration-path-loses-its-sibling-includes) is in the same
version and is **done**: it needed no decision behind it, because a configuration path with no
parent directory losing the files it includes is a defect rather than a choice.
All three are done, on one branch: they change the same loop and P41 cannot ship without P40, so
they are one piece of work rather than three batched together.
[P42](post-v1.md#p42--an-empty-configuration-and-the-layer-that-empties-it) joined them in 0.2.0
